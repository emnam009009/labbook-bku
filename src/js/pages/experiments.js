  ...
  tbody.innerHTML = rows.length
    ? rows.map(r =>
        ...
        '<td class="action-cell" onclick="event.stopPropagation()" style="text-align:left">' +
          '<label class="exp-bar" onclick="event.stopPropagation();openExpActionsMenu(this,{refType:\'hydro\',refId:\'' + r._key + '\',code:\'' + escapeJs(r.code || r._key) + '\'})" title="Thao tác" aria-label="Menu thao tác thí nghiệm"><input type="checkbox" tabindex="-1"><span class="exp-bar-top"></span><span class="exp-bar-middle"></span><span class="exp-bar-bottom"></span></label>' + '<label class="lock-toggle hydro-lock-btn" style="display:none" onclick="event.stopPropagation();' + (r.locked ? 'unlockItem(\'hydro\',\'' + r._key + '\')' : 'lockItem(\'hydro\',\'' + r._key + '\')') + '">' +
...
export function renderElectrode() {
  ...
  tbody.innerHTML = rows.length
    ? rows.map(r =>
        ...
        '<td class="action-cell" onclick="event.stopPropagation()" style="text-align:left">' +
          '<label class="exp-bar" onclick="event.stopPropagation();openExpActionsMenu(this,{refType:\'electrode\',refId:\'' + r._key + '\',code:\'' + r.code + '\'})" title="Thao tác" aria-label="Menu thao tác điện cực"><input type="checkbox" tabindex="-1"><span class="exp-bar-top"></span><span class="exp-bar-middle"></span><span class="exp-bar-bottom"></span></label>' + '<label class="lock-toggle electrode-lock-btn" style="display:none" onclick="event.stopPropagation();' + (r.locked ? 'unlockItem(\'electrode\',\'' + r._key + '\')' : 'lockItem(\'electrode\',\'' + r._key + '\')') + '">' +
            '<div class="lock-track ' + (r.locked ? 'locked' : 'unlocked') + '">' +
              '<span class="lock-icon">' + (r.locked ? LOCK_ICON_ON : LOCK_ICON_OFF) + '</span><div class="lock-thumb"></div>' +
            '</div>' +
          '</label>' +
