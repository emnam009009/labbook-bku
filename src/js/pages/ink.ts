...
export function renderInk(): void {
  ...
  tbody.innerHTML = rows.map(function(r: any) {
    ...
    return '<tr class="clickable-row" style="vertical-align:middle" onclick="' + (!r.locked ? 'editInk(\'' + safeKey + '\')' : '') + '" title="' + (!r.locked ? 'Nhan de sua' : 'Da khoa') + '">' +
      ...
      '<td class="action-cell" onclick="event.stopPropagation()" style="text-align:left">' +
        '<label class="lock-toggle ink-lock-btn" style="display:none" onclick="event.stopPropagation();' + (r.locked ? 'unlockItem(\'ink\',\'' + safeKey + '\')' : 'lockItem(\'ink\',\'' + safeKey + '\')') + '">' +
          ...
          '</div>' +
        '</label>' +
        '<button class="plusButton" onclick="duplicateItem(\'ink\',\'' + safeKey + '\')" title="Nhan ban"><svg class="plusIcon" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M12 5v14M5 12h14"/></svg></button>' +
