...
function buildDocDefinition(data, sections, orientation, isAdmin) {
  ...
  const exporterName = window.currentAuth?.displayName || window.currentUser || data.currentUserName || 'Không rõ'
  const exporterEmail = window.currentAuth?.email || ''
  const exporterRole = window.currentAuth?.role || ''
  const roleLabel = exporterRole === 'superadmin' ? 'Super Admin' : exporterRole === 'admin' ? 'Admin' : exporterRole === 'member' ? 'Thành viên' : 'Người dùng'
  ...
  content.push({
    columns: [
      ...
      {
        stack: [
          { text: 'Xuất lúc', style: 'caption', alignment: 'right' },
          { text: dateStr, style: 'datestamp', alignment: 'right', margin: [0, 2, 0, 0] },
          { text: timeStr, style: 'datestampSmall', alignment: 'right', margin: [0, 1, 0, 6] },
          { text: 'Người xuất', style: 'caption', alignment: 'right' },
          { text: exporterName, style: 'datestamp', alignment: 'right', margin: [0, 2, 0, 0] },
          { text: roleLabel, style: 'datestampSmall', alignment: 'right', margin: [0, 1, 0, 0] },
        ],
        width: 160,
      },
    ],
    ...
    footer: function(currentPage, pageCount) {
      return {
        columns: [
          {
            stack: [
              { text: `Generated by LabBook BKU — labbook-bku.web.app`, style: 'footer' },
              { text: `Xuất bởi ${exporterName} (${roleLabel})${exporterEmail ? ' — ' + exporterEmail : ''} · ${dateStr} ${timeStr}`, style: 'footer', margin: [0, 2, 0, 0] },
            ],
            margin: [40, 0, 0, 0],
          },
          { text: `Trang ${currentPage}/${pageCount}`, style: 'footer', alignment: 'right', margin: [0, 12, 40, 0], width: 80 },
        ],
      }
    },
    styles: {
