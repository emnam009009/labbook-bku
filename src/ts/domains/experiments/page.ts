...
import { listExperiments } from "@/domains/experiments/service";
...
} from "@/domains/data-assets/service";
// R154-1: Lineage graph
import { buildLineageGraph } from "@/domains/lineage/service";
...
async function ensureSamplePickerCache(): Promise<any[]> {
  ...
  try {
    const { listSamples } = await import("@/domains/samples/service");
...
export async function submitExperimentForm(): Promise<void> {
  ...
  try {
    const { createExperiment } = await import("@/domains/experiments/service");
    const input: any = {
      type,
...
export async function handleDataAssetDownload(assetId: string): Promise<void> {
  try {
    ...
    const { getDataAsset } = await import('@/domains/data-assets/service');
